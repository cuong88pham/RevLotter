export default `
<html xmlns="http://www.w3.org/1999/xhtml">
  <link type="text/css" id="dark-mode" rel="stylesheet" href="" />
  <style type="text/css" id="dark-mode-custom-style"></style>
  <head>
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <!--[if !mso]><!-->
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <!--<![endif]-->
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta name="x-apple-disable-message-reformatting" />
    <title>Email Tempalte</title>
    <style type="text/css">
      @import url(
        https://fonts.googleapis.com/css?family=Nunito:200,300,400,600,700,800,900|Raleway:200,300,400,500,600,700,800,900
      );

      html {
        width: 100%;
      }
      body {
        -webkit-text-size-adjust: none;
        -ms-text-size-adjust: none;
        margin: 0;
        padding: 0;
        font-family: "Open Sans", Arial, Sans-serif !important;
      }
      table {
        border-spacing: 0;
        table-layout: auto;
        margin: 0 auto;
      }
      img {
        display: block !important;
        overflow: hidden !important;
      }
      a {
        text-decoration: none;
        color: unset;
      }
      .ReadMsgBody {
        width: 100%;
        background-color: #fff;
      }
      .ExternalClass {
        width: 100%;
        background-color: #fff;
      }
      .ExternalClass,
      .ExternalClass p,
      .ExternalClass span,
      .ExternalClass font,
      .ExternalClass td,
      .ExternalClass div {
        line-height: 100%;
      }
      .yshortcuts a {
        border-bottom: none !important;
      }
      .full {
        width: 100%;
      }
      .pad {
        width: 92%;
      }
      @media only screen and (max-width: 650px) {
        .res-pad {
          width: 92%;
          max-width: 92%;
        }
        .res-full {
          width: 100%;
          max-width: 100%;
        }
        .res-left {
          text-align: left !important;
        }
        .res-right {
          text-align: right !important;
        }
        .res-center {
          text-align: center !important;
        }
      }
      @media only screen and (max-width: 750px) {
        .margin-full {
          width: 100%;
          max-width: 100%;
        }
        .margin-pad {
          width: 92%;
          max-width: 92%;
          max-width: 600px;
        }
      }
    </style>
  </head>
  <body>
    <!-- ====== Module : Header ====== -->

    <table
      class="full"
      cellspacing="0"
      cellpadding="0"
      border="0"
      bgcolor="#F5F5F5"
      align="center"
    >
      <tbody>
        <tr>
          <td>
            <table
              class="margin-full"
              width="750"
              cellspacing="0"
              cellpadding="0"
              border="0"
              bgcolor="white"
              align="center"
            >
              <tbody>
                <tr>
                  <td>
                    <table
                      class="margin-pad"
                      width="600"
                      cellspacing="0"
                      cellpadding="0"
                      border="0"
                      align="center"
                    >
                      <tbody>
                        <tr>
                          <td style="font-size:0px" height="30">&nbsp;</td>
                        </tr>
                        <!-- column x2 -->
                        <tr>
                          <td>
                            <table
                              class="full"
                              cellspacing="0"
                              cellpadding="0"
                              border="0"
                              align="center"
                            >
                              <tbody>
                                <tr>
                                  <td valign="top">
                                    <!-- left column -->
                                    <table
                                      class="res-full"
                                      width="126"
                                      cellspacing="0"
                                      cellpadding="0"
                                      border="0"
                                      align="left"
                                    >
                                      <!-- image -->
                                      <tbody>
                                        <tr>
                                          <td>
                                            <table
                                              class="res-full"
                                              cellspacing="0"
                                              cellpadding="0"
                                              border="0"
                                              align="left"
                                            >
                                              <tbody>
                                                <tr>
                                                  <td>
                                                    <table
                                                      cellspacing="0"
                                                      cellpadding="0"
                                                      border="0"
                                                      align="center"
                                                    >
                                                      <tbody>
                                                        <tr>
                                                          <td>
                                                            <svg height="48" viewBox="0 0 268 48" fill="none" xmlns="http://www.w3.org/2000/svg" width="146">
                                                            <g clip-path="url(#clip0)">
                                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M4.14802 11.1129C3.15737 11.1129 2.34933 11.9201 2.34933 12.9102V44.0675C2.34933 45.0588 3.15716 45.8662 4.14802 45.8662H70.1911C71.1826 45.8662 71.9884 45.0595 71.9884 44.0675V12.9102C71.9884 11.9194 71.1824 11.1129 70.1911 11.1129H4.14802ZM70.1911 47.9777H4.14802C1.9921 47.9777 0.237793 46.2234 0.237793 44.0675V12.9102C0.237793 10.7543 1.9921 9 4.14802 9H70.1911C72.347 9 74.1013 10.7543 74.1013 12.9102V44.0675C74.1013 46.2234 72.347 47.9777 70.1911 47.9777Z" fill="#6D48E5" stroke="#6D48E5" stroke-width="0.5"></path>
                                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M81.8447 2.24338C79.9676 2.24338 78.4423 3.77031 78.4423 5.64718C78.4423 7.52377 79.9673 9.04959 81.8447 9.04959C83.7203 9.04959 85.2471 7.5228 85.2471 5.64718C85.2471 3.77128 83.7201 2.24338 81.8447 2.24338ZM81.8447 11.1625C78.8024 11.1625 76.328 8.68856 76.328 5.64718C76.328 2.6058 78.8024 0.131836 81.8447 0.131836C84.8856 0.131836 87.3587 2.60576 87.3587 5.64718C87.3587 8.6886 84.8856 11.1625 81.8447 11.1625ZM16.3405 25.3307H14.9435V25.5549C14.9435 26.1393 14.4717 26.6121 13.8877 26.6121H10.772C10.1878 26.6121 9.71484 26.1391 9.71484 25.5549V20.1809C9.71484 19.5967 10.1878 19.1237 10.772 19.1237H22.2677C22.8507 19.1237 23.3234 19.597 23.3234 20.1809V24.275C23.3234 24.8577 22.8504 25.3307 22.2677 25.3307C21.6858 25.3307 21.2119 24.8573 21.2119 24.275V21.2367H11.8278V24.4991H12.832V24.275C12.832 23.691 13.3047 23.2178 13.8877 23.2178H18.4446C18.8515 23.2178 19.2234 23.454 19.3967 23.8227C19.5721 24.1929 19.519 24.6289 19.2588 24.9438C16.4905 28.3218 15.2004 31.9237 14.8917 35.1346H18.3205C18.3511 31.7279 18.7185 28.9652 19.4144 26.9134C19.6045 26.3596 20.2013 26.0643 20.7548 26.2523C21.3073 26.4404 21.603 27.0406 21.416 27.5927C20.7394 29.5825 20.4079 32.3964 20.4315 35.9561L20.4342 36.1903C20.4342 36.7729 19.9601 37.2461 19.3771 37.2461H13.7835C13.2121 37.2461 12.7457 36.7947 12.7264 36.2242C12.7243 36.1523 12.7237 36.0487 12.7266 35.9098C12.731 35.697 12.7428 35.4552 12.764 35.1864C12.8227 34.4413 12.9425 33.6148 13.1391 32.7245C13.6841 30.2562 14.711 27.7463 16.3405 25.3307ZM36.9902 25.3307H35.5932V25.5549C35.5932 26.1388 35.1204 26.6121 34.5374 26.6121H31.4203C30.8382 26.6121 30.3645 26.1384 30.3645 25.5549V20.1809C30.3645 19.5974 30.8382 19.1237 31.4203 19.1237H42.9173C43.5003 19.1237 43.9731 19.597 43.9731 20.1809V24.275C43.9731 24.8577 43.5001 25.3307 42.9173 25.3307C42.3346 25.3307 41.8616 24.8577 41.8616 24.275V21.2367H32.4774V24.4991H33.4816V24.275C33.4816 23.691 33.9544 23.2178 34.5374 23.2178H39.0929C39.5009 23.2178 39.873 23.4538 40.0464 23.8227C40.2212 24.1917 40.1681 24.6277 39.9098 24.9438C37.1394 28.3203 35.8534 31.9126 35.5414 35.1346H38.9687C38.9994 31.7251 39.3673 28.9635 40.0654 26.9137C40.2526 26.3595 40.8482 26.0644 41.4045 26.2523C41.9568 26.4409 42.2538 27.0418 42.0657 27.5927C41.3876 29.5825 41.0562 32.3964 41.0811 35.9565L41.0825 36.1903C41.0825 36.7726 40.6086 37.2461 40.0267 37.2461H34.4304C33.8625 37.2461 33.3954 36.794 33.3746 36.2238C33.3728 36.1432 33.3724 36.0475 33.3754 35.9098C33.38 35.6966 33.392 35.455 33.4133 35.1863C33.4724 34.4409 33.5924 33.6146 33.7891 32.7245C34.3346 30.2559 35.3614 27.7462 36.9902 25.3307ZM57.6413 25.3307H56.2498V25.5549C56.2498 26.1381 55.7749 26.6121 55.1926 26.6121H52.0755C51.4934 26.6121 51.0197 26.1384 51.0197 25.5549V20.1809C51.0197 19.5974 51.4934 19.1237 52.0755 19.1237H63.5712C64.1533 19.1237 64.6269 19.5974 64.6269 20.1809V24.275C64.6269 24.8573 64.153 25.3307 63.5712 25.3307C62.9882 25.3307 62.514 24.8575 62.514 24.275V21.2367H53.1313V24.4991H54.1369V24.275C54.1369 23.691 54.6096 23.2178 55.1926 23.2178H59.7467C60.1545 23.2178 60.5266 23.4536 60.7016 23.8227C60.8764 24.1917 60.8233 24.6277 60.5651 24.9438C57.7935 28.3215 56.5072 31.9132 56.1952 35.1346H59.624C59.6546 31.7229 60.0213 28.963 60.718 26.9132C60.9089 26.3612 61.5072 26.0666 62.0583 26.2523C62.6108 26.4404 62.9065 27.0406 62.7195 27.5928C62.0417 29.5764 61.7113 32.3865 61.735 35.9547L61.7377 36.1903C61.7377 36.7729 61.2636 37.2461 60.6806 37.2461H55.087C54.516 37.2461 54.0492 36.7951 54.0299 36.2242C54.0278 36.152 54.0272 36.0487 54.03 35.9099C54.0344 35.697 54.0461 35.4552 54.0673 35.1864C54.1258 34.4413 54.2454 33.6148 54.4417 32.7245C54.9861 30.2562 56.0124 27.7463 57.6413 25.3307Z" fill="#6D48E5" stroke="#6D48E5" stroke-width="0.5"></path>
                                                            <path fill-rule="evenodd" clip-rule="evenodd" d="M80.7883 26.6687V12.0305C80.7883 11.4468 81.2608 10.9747 81.8441 10.9747C82.4273 10.9747 82.8998 11.4468 82.8998 12.0305V27.7259C82.8998 28.3086 82.4268 28.7816 81.8441 28.7816H79.5288C78.9472 28.7816 78.4744 28.3084 78.4744 27.7259C78.4744 27.1422 78.947 26.6687 79.5288 26.6687H80.7883ZM75.4115 32.1984H75.4778L75.4115 32.2646V32.1984ZM75.4115 32.1984V23.2534H74.9219C74.3394 23.2534 73.8662 22.7792 73.8662 22.1962C73.8662 21.6134 74.3392 21.1404 74.9219 21.1404H76.4673C77.0512 21.1404 77.5244 21.6132 77.5244 22.1962V33.2541C77.5244 33.8374 77.051 34.3113 76.4673 34.3113H75.3986C74.814 34.3113 74.3414 33.8378 74.3414 33.2541C74.3414 32.6707 74.8137 32.1984 75.3986 32.1984H75.4115ZM7.46205 16.252V40.1169H23.1634C23.7455 40.1169 24.2191 40.5906 24.2191 41.1741C24.2191 41.7564 23.7452 42.2298 23.1634 42.2298H6.40489C5.82304 42.2298 5.34912 41.7564 5.34912 41.1741V15.1963C5.34912 14.6137 5.82328 14.1391 6.40489 14.1391H23.1634C23.745 14.1391 24.2191 14.6137 24.2191 15.1963C24.2191 15.7786 23.7452 16.252 23.1634 16.252H7.46205ZM27.9853 40.1169H46.3552V16.2506H27.9853V40.1169ZM47.411 42.2298H26.9295C26.3465 42.2298 25.8723 41.7566 25.8723 41.1741V15.1963C25.8723 14.6135 26.3467 14.1391 26.9295 14.1391H47.411C47.9947 14.1391 48.4682 14.613 48.4682 15.1963V41.1741C48.4682 41.7571 47.9949 42.2298 47.411 42.2298ZM66.8381 40.1169V16.252H51.141C50.5582 16.252 50.0852 15.779 50.0852 15.1963C50.0852 14.6132 50.5584 14.1391 51.141 14.1391H67.8967C68.4801 14.1391 68.9525 14.6128 68.9525 15.1963V41.1741C68.9525 41.7573 68.4804 42.2298 67.8967 42.2298H51.141C50.5582 42.2298 50.0852 41.7568 50.0852 41.1741C50.0852 40.5901 50.558 40.1169 51.141 40.1169H66.8381Z" fill="#6D48E5" stroke="#6D48E5" stroke-width="0.5"></path>
                                                            </g>
                                                            <path d="M106.104 23.776H102.072V21.4H113.052V23.776H109.02V34H106.104V23.776ZM120.309 24.172C121.509 24.172 122.475 24.532 123.207 25.252C123.951 25.972 124.323 27.04 124.323 28.456V34H121.515V28.888C121.515 28.12 121.347 27.55 121.011 27.178C120.675 26.794 120.189 26.602 119.553 26.602C118.845 26.602 118.281 26.824 117.861 27.268C117.441 27.7 117.231 28.348 117.231 29.212V34H114.423V20.644H117.231V25.324C117.603 24.952 118.053 24.67 118.581 24.478C119.109 24.274 119.685 24.172 120.309 24.172ZM136.46 29.194C136.46 29.23 136.442 29.482 136.406 29.95H129.08C129.212 30.55 129.524 31.024 130.016 31.372C130.508 31.72 131.12 31.894 131.852 31.894C132.356 31.894 132.8 31.822 133.184 31.678C133.58 31.522 133.946 31.282 134.282 30.958L135.776 32.578C134.864 33.622 133.532 34.144 131.78 34.144C130.688 34.144 129.722 33.934 128.882 33.514C128.042 33.082 127.394 32.488 126.938 31.732C126.482 30.976 126.254 30.118 126.254 29.158C126.254 28.21 126.476 27.358 126.92 26.602C127.376 25.834 127.994 25.24 128.774 24.82C129.566 24.388 130.448 24.172 131.42 24.172C132.368 24.172 133.226 24.376 133.994 24.784C134.762 25.192 135.362 25.78 135.794 26.548C136.238 27.304 136.46 28.186 136.46 29.194ZM131.438 26.296C130.802 26.296 130.268 26.476 129.836 26.836C129.404 27.196 129.14 27.688 129.044 28.312H133.814C133.718 27.7 133.454 27.214 133.022 26.854C132.59 26.482 132.062 26.296 131.438 26.296ZM149.655 34.216C148.371 34.216 147.207 33.94 146.163 33.388C145.131 32.824 144.315 32.05 143.715 31.066C143.127 30.07 142.833 28.948 142.833 27.7C142.833 26.452 143.127 25.336 143.715 24.352C144.315 23.356 145.131 22.582 146.163 22.03C147.207 21.466 148.377 21.184 149.673 21.184C150.765 21.184 151.749 21.376 152.625 21.76C153.513 22.144 154.257 22.696 154.857 23.416L152.985 25.144C152.133 24.16 151.077 23.668 149.817 23.668C149.037 23.668 148.341 23.842 147.729 24.19C147.117 24.526 146.637 25 146.289 25.612C145.953 26.224 145.785 26.92 145.785 27.7C145.785 28.48 145.953 29.176 146.289 29.788C146.637 30.4 147.117 30.88 147.729 31.228C148.341 31.564 149.037 31.732 149.817 31.732C151.077 31.732 152.133 31.234 152.985 30.238L154.857 31.966C154.257 32.698 153.513 33.256 152.625 33.64C151.737 34.024 150.747 34.216 149.655 34.216ZM159.31 25.594C159.646 25.126 160.096 24.772 160.66 24.532C161.236 24.292 161.896 24.172 162.64 24.172V26.764C162.328 26.74 162.118 26.728 162.01 26.728C161.206 26.728 160.576 26.956 160.12 27.412C159.664 27.856 159.436 28.528 159.436 29.428V34H156.628V24.316H159.31V25.594ZM174.256 24.316L169.882 34.594C169.438 35.71 168.886 36.496 168.226 36.952C167.578 37.408 166.792 37.636 165.868 37.636C165.364 37.636 164.866 37.558 164.374 37.402C163.882 37.246 163.48 37.03 163.168 36.754L164.194 34.756C164.41 34.948 164.656 35.098 164.932 35.206C165.22 35.314 165.502 35.368 165.778 35.368C166.162 35.368 166.474 35.272 166.714 35.08C166.954 34.9 167.17 34.594 167.362 34.162L167.398 34.072L163.204 24.316H166.102L168.82 30.886L171.556 24.316H174.256ZM181.125 24.172C182.025 24.172 182.841 24.382 183.573 24.802C184.317 25.21 184.899 25.792 185.319 26.548C185.739 27.292 185.949 28.162 185.949 29.158C185.949 30.154 185.739 31.03 185.319 31.786C184.899 32.53 184.317 33.112 183.573 33.532C182.841 33.94 182.025 34.144 181.125 34.144C179.889 34.144 178.917 33.754 178.209 32.974V37.492H175.401V24.316H178.083V25.432C178.779 24.592 179.793 24.172 181.125 24.172ZM180.639 31.84C181.359 31.84 181.947 31.6 182.403 31.12C182.871 30.628 183.105 29.974 183.105 29.158C183.105 28.342 182.871 27.694 182.403 27.214C181.947 26.722 181.359 26.476 180.639 26.476C179.919 26.476 179.325 26.722 178.857 27.214C178.401 27.694 178.173 28.342 178.173 29.158C178.173 29.974 178.401 30.628 178.857 31.12C179.325 31.6 179.919 31.84 180.639 31.84ZM194.075 33.532C193.799 33.736 193.457 33.892 193.049 34C192.653 34.096 192.233 34.144 191.789 34.144C190.637 34.144 189.743 33.85 189.107 33.262C188.483 32.674 188.171 31.81 188.171 30.67V26.692H186.677V24.532H188.171V22.174H190.979V24.532H193.391V26.692H190.979V30.634C190.979 31.042 191.081 31.36 191.285 31.588C191.501 31.804 191.801 31.912 192.185 31.912C192.629 31.912 193.007 31.792 193.319 31.552L194.075 33.532ZM199.961 34.144C198.941 34.144 198.023 33.934 197.207 33.514C196.403 33.082 195.773 32.488 195.317 31.732C194.861 30.976 194.633 30.118 194.633 29.158C194.633 28.198 194.861 27.34 195.317 26.584C195.773 25.828 196.403 25.24 197.207 24.82C198.023 24.388 198.941 24.172 199.961 24.172C200.981 24.172 201.893 24.388 202.697 24.82C203.501 25.24 204.131 25.828 204.587 26.584C205.043 27.34 205.271 28.198 205.271 29.158C205.271 30.118 205.043 30.976 204.587 31.732C204.131 32.488 203.501 33.082 202.697 33.514C201.893 33.934 200.981 34.144 199.961 34.144ZM199.961 31.84C200.681 31.84 201.269 31.6 201.725 31.12C202.193 30.628 202.427 29.974 202.427 29.158C202.427 28.342 202.193 27.694 201.725 27.214C201.269 26.722 200.681 26.476 199.961 26.476C199.241 26.476 198.647 26.722 198.179 27.214C197.711 27.694 197.477 28.342 197.477 29.158C197.477 29.974 197.711 30.628 198.179 31.12C198.647 31.6 199.241 31.84 199.961 31.84ZM212.443 21.4H215.359V31.624H221.677V34H212.443V21.4ZM227.646 34.144C226.626 34.144 225.708 33.934 224.892 33.514C224.088 33.082 223.458 32.488 223.002 31.732C222.546 30.976 222.318 30.118 222.318 29.158C222.318 28.198 222.546 27.34 223.002 26.584C223.458 25.828 224.088 25.24 224.892 24.82C225.708 24.388 226.626 24.172 227.646 24.172C228.666 24.172 229.578 24.388 230.382 24.82C231.186 25.24 231.816 25.828 232.272 26.584C232.728 27.34 232.956 28.198 232.956 29.158C232.956 30.118 232.728 30.976 232.272 31.732C231.816 32.488 231.186 33.082 230.382 33.514C229.578 33.934 228.666 34.144 227.646 34.144ZM227.646 31.84C228.366 31.84 228.954 31.6 229.41 31.12C229.878 30.628 230.112 29.974 230.112 29.158C230.112 28.342 229.878 27.694 229.41 27.214C228.954 26.722 228.366 26.476 227.646 26.476C226.926 26.476 226.332 26.722 225.864 27.214C225.396 27.694 225.162 28.342 225.162 29.158C225.162 29.974 225.396 30.628 225.864 31.12C226.332 31.6 226.926 31.84 227.646 31.84ZM241.079 33.532C240.803 33.736 240.461 33.892 240.053 34C239.657 34.096 239.237 34.144 238.793 34.144C237.641 34.144 236.747 33.85 236.111 33.262C235.487 32.674 235.175 31.81 235.175 30.67V26.692H233.681V24.532H235.175V22.174H237.983V24.532H240.395V26.692H237.983V30.634C237.983 31.042 238.085 31.36 238.289 31.588C238.505 31.804 238.805 31.912 239.189 31.912C239.633 31.912 240.011 31.792 240.323 31.552L241.079 33.532ZM248.69 33.532C248.414 33.736 248.072 33.892 247.664 34C247.268 34.096 246.848 34.144 246.404 34.144C245.252 34.144 244.358 33.85 243.722 33.262C243.098 32.674 242.786 31.81 242.786 30.67V26.692H241.292V24.532H242.786V22.174H245.594V24.532H248.006V26.692H245.594V30.634C245.594 31.042 245.696 31.36 245.9 31.588C246.116 31.804 246.416 31.912 246.8 31.912C247.244 31.912 247.622 31.792 247.934 31.552L248.69 33.532ZM259.454 29.194C259.454 29.23 259.436 29.482 259.4 29.95H252.074C252.206 30.55 252.518 31.024 253.01 31.372C253.502 31.72 254.114 31.894 254.846 31.894C255.35 31.894 255.794 31.822 256.178 31.678C256.574 31.522 256.94 31.282 257.276 30.958L258.77 32.578C257.858 33.622 256.526 34.144 254.774 34.144C253.682 34.144 252.716 33.934 251.876 33.514C251.036 33.082 250.388 32.488 249.932 31.732C249.476 30.976 249.248 30.118 249.248 29.158C249.248 28.21 249.47 27.358 249.914 26.602C250.37 25.834 250.988 25.24 251.768 24.82C252.56 24.388 253.442 24.172 254.414 24.172C255.362 24.172 256.22 24.376 256.988 24.784C257.756 25.192 258.356 25.78 258.788 26.548C259.232 27.304 259.454 28.186 259.454 29.194ZM254.432 26.296C253.796 26.296 253.262 26.476 252.83 26.836C252.398 27.196 252.134 27.688 252.038 28.312H256.808C256.712 27.7 256.448 27.214 256.016 26.854C255.584 26.482 255.056 26.296 254.432 26.296ZM264.005 25.594C264.341 25.126 264.791 24.772 265.355 24.532C265.931 24.292 266.591 24.172 267.335 24.172V26.764C267.023 26.74 266.813 26.728 266.705 26.728C265.901 26.728 265.271 26.956 264.815 27.412C264.359 27.856 264.131 28.528 264.131 29.428V34H261.323V24.316H264.005V25.594Z" fill="#6D48E5"></path>
                                                            <defs>
                                                            <clipPath id="clip0">
                                                            <rect width="87.5138" height="48" fill="white"></rect>
                                                            </clipPath>
                                                            </defs>
                                                            <link xmlns="" type="text/css" id="dark-mode" rel="stylesheet" href=""></link><style xmlns="" type="text/css" id="dark-mode-custom-style"></style></svg>
                                                          </td>
                                                        </tr>
                                                      </tbody>
                                                    </table>
                                                  </td>
                                                </tr>
                                              </tbody>
                                            </table>
                                          </td>
                                        </tr>
                                        <!-- image end -->
                                      </tbody>
                                    </table>
                                    <!-- left column end -->
                                    <!--[if (gte mso 9)|(IE)]></td><td><![endif]-->
                                    <table
                                      class="res-full"
                                      width="1"
                                      cellspacing="0"
                                      cellpadding="0"
                                      border="0"
                                      align="left"
                                    >
                                      <tbody>
                                        <tr>
                                          <td style="font-size:0px" height="1">
                                            &nbsp;
                                          </td>
                                        </tr>
                                      </tbody>
                                    </table>
                                    <!--[if (gte mso 9)|(IE)]></td><td valign="top"><![endif]-->
                                    <!-- right column -->

                                    <!-- right column end -->
                                  </td>
                                </tr>
                              </tbody>
                            </table>
                          </td>
                        </tr>
                        <!-- column x2 end -->
                        <tr>
                          <td style="font-size:0px" height="30">&nbsp;</td>
                        </tr>
                      </tbody>
                    </table>
                  </td>
                </tr>
              </tbody>
            </table>
          </td>
        </tr>
      </tbody>
    </table>
    <!-- ====== Module : Intro ====== -->
    <table
      class="full"
      cellspacing="0"
      cellpadding="0"
      border="0"
      bgcolor="#F5F5F5"
      align="center"
    >
      <tbody>
        <tr>
          <td>
            <table
              class="margin-full"
              width="750"
              cellspacing="0"
              cellpadding="0"
              border="0"
              bgcolor="white"
              align="center"
            >
              <tbody>
                <tr>
                  <td>
                    <table
                      class="margin-pad"
                      width="600"
                      cellspacing="0"
                      cellpadding="0"
                      border="0"
                      align="center"
                    >
                      <tbody>
                        <tr>
                          <td style="font-size:0px" height="70">&nbsp;</td>
                        </tr>
                        <!-- subtitle -->
                        <tr>
                          <td
                            class="res-center"
                            style="text-align: center; color: #6d48e5; font-family: 'Nunito', Arial, Sans-serif; font-size: 14px; letter-spacing: 0.7px; word-break: break-word; font-weight: 600;"
                          >
                            Subscribe successfull!
                          </td>
                        </tr>
                        <!-- subtitle end -->
                        <tr>
                          <td style="font-size:0px" height="7">&nbsp;</td>
                        </tr>
                        <!-- title -->
                        <tr>
                          <td
                            class="res-center"
                            style="text-align: center; color: #505050; font-family: 'Raleway', Arial, Sans-serif; font-size: 22px; letter-spacing: 0.7px; word-break: break-word"
                          >
                            Thank you very much! <br />We will send you
                            notifications when we go live.
                          </td>
                        </tr>
                        <!-- title end -->
                        <tr>
                          <td style="font-size:0px" height="13">&nbsp;</td>
                        </tr>
                        <!-- dash -->
                        <tr>
                          <td>
                            <table
                              class="res-full"
                              cellspacing="0"
                              cellpadding="0"
                              border="0"
                              align="center"
                            >
                              <tbody>
                                <tr>
                                  <td>
                                    <table
                                      style="border-radius: 10px;"
                                      cellspacing="0"
                                      cellpadding="0"
                                      border="0"
                                      bgcolor="#6d48e5"
                                      align="center"
                                    >
                                      <tbody>
                                        <tr>
                                          <td
                                            style="font-size:0px"
                                            width="50"
                                            height="3"
                                          >
                                            &nbsp;
                                          </td>
                                        </tr>
                                      </tbody>
                                    </table>
                                  </td>
                                </tr>
                              </tbody>
                            </table>
                          </td>
                        </tr>
                        <!-- dash end -->
                        <tr>
                          <td style="font-size:0px" height="25">&nbsp;</td>
                        </tr>
                        <!-- column x3 -->

                        <!-- column x3 end -->
                        <tr>
                          <td style="font-size:0px" height="70">&nbsp;</td>
                        </tr>
                      </tbody>
                    </table>
                  </td>
                </tr>
              </tbody>
            </table>
          </td>
        </tr>
      </tbody>
    </table>
    <!-- ====== Module : Footer ====== -->

    <!-- ====== Module : Unsubscribe ====== -->
    <table
      class="full"
      cellspacing="0"
      cellpadding="0"
      border="0"
      bgcolor="#F5F5F5"
      align="center"
    >
      <tbody>
        <tr>
          <td>
            <table
              class="margin-full"
              width="750"
              cellspacing="0"
              cellpadding="0"
              border="0"
              bgcolor="#F5F5F5"
              align="center"
            >
              <tbody>
                <tr>
                  <td>
                    <table
                      class="margin-pad"
                      width="600"
                      cellspacing="0"
                      cellpadding="0"
                      border="0"
                      align="center"
                    >
                      <tbody>
                        <tr>
                          <td style="font-size:0px" height="50">&nbsp;</td>
                        </tr>
                        <tr>
                          <td>
                            <table
                              cellspacing="0"
                              cellpadding="0"
                              border="0"
                              align="center"
                            >
                              <tbody>
                                <tr>
                                  <td>
                                    <table
                                      class="full"
                                      cellspacing="0"
                                      cellpadding="0"
                                      border="0"
                                      align="center"
                                    >
                                      <!-- paragraph -->
                                      <tbody>
                                        <tr>
                                          <td
                                            class="res-center"
                                            style="text-align: center; color: #909090; font-family: 'Nunito', Arial, Sans-serif; letter-spacing: 0.4px; line-height: 23px; word-break: break-word; font-size: 15px;"
                                          >
                                            © The Crypto Lottery 2020
                                            Rights Reserved
                                          </td>
                                        </tr>
                                        <!-- paragraph end -->
                                      </tbody>
                                    </table>
                                  </td>
                                  <td>
                                    <table
                                      class="full"
                                      cellspacing="0"
                                      cellpadding="0"
                                      border="0"
                                      align="center"
                                    >
                                      <!-- paragraph -->
                                      <tbody>
                                        <tr>
                                          <td
                                            class="res-center"
                                            style="text-align: center; color: #304050; font-family: 'Nunito', Arial, Sans-serif; font-size: 17px; letter-spacing: 0.4px; line-height: 23px; word-break: break-word; padding-left: 7px; font-weight: 600;"
                                          ></td>
                                        </tr>
                                        <!-- paragraph end -->
                                      </tbody>
                                    </table>
                                  </td>
                                </tr>
                              </tbody>
                            </table>
                          </td>
                        </tr>
                        <tr>
                          <td style="font-size:0px" height="50">&nbsp;</td>
                        </tr>
                      </tbody>
                    </table>
                  </td>
                </tr>
              </tbody>
            </table>
          </td>
        </tr>
      </tbody>
    </table>
  </body>
</html>
`;
